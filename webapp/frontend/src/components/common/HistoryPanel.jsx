...
const HistoryPanel = ({ history, onRestore, onRemove, onClear, isOpen, onToggle }) => {
  ...
  const getTypeBadgeColor = (type) => {
    const colors = {
      cfn: 'bg-orange-500/20 text-orange-400 border-orange-500/30',
      ...
      <motion.button
        ...
        className={`fixed top-20 right-4 z-40 p-3 rounded-lg backdrop-blur-sm border transition-colors ${
          isOpen
            ...
            : 'bg-[#FF9900] border-[#e68a00] text-[#0f172a] shadow-lg'
        }`}
                ...
                {history.length === 0 ? (
                  ...
                  history.map((item) => (
                    <motion.div
                      ...
                      className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-4 hover:border-orange-500/50 transition-all cursor-pointer group"
                    >
                      {/* Header */}
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2 px-3 bg-[#FF9900] hover:bg-[#e68a00] rounded-lg text-sm font-semibold text-[#0f172a] transition-all"
                      >
